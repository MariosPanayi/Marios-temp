...
for i = 1: size(data,2)
    ...
    temp_mean = mean(tempData);
    avg_DA(i,:) = temp_mean;
    
    temp_Data_Cal = temp_mean/data(i).calibrationFactor;
    avg_DA_cal(i,:) = temp_Data_Cal;
    
    if strcmp([data(i).genotype], 'WT');
        avg_DA_WT = [avg_DA_WT;  temp_mean];
        avg_DA_cal_WT = [avg_DA_cal_WT;  temp_Data_Cal];
    elseif strcmp([data(i).genotype], 'KO');
        avg_DA_KO = [avg_DA_KO; temp_mean];
        avg_DA_cal_KO = [avg_DA_cal_KO; temp_Data_Cal];
    end
end


%%
save('F:\Marios aFCV\GLRA_002\DataAnalysis\GLRA002_06DrugPeriodData', 'data', 'summary', 'avg_DA_WT','avg_DA_cal_WT' , 'avg_DA_KO','avg_DA_cal_KO')
...
%DA data picker for even rows
tempData = tempData(1:2:size(tempData,1),:);
%Prune to 18 traces where expt. happened
tempData = tempData(1:18,:);
